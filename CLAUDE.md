...
Note `ThorHudClient` must remove `VanillaHudElements.AIR_BAR` alongside the others; missing it leaves the game drawing its own bubbles while the second screen draws them too.
...
- `CommandDispatcher` — maps letter codes to `KeyBinding`s and digit strings to hotbar slots. Hotbar taps go through the vanilla `hotbarKeys[]` binding rather than setting the inventory slot directly, so `MinecraftClient.handleInputEvents` keeps handling spectator mode, the creative toolbar modifiers, and server sync.
  ...
  - **The `COMMANDS` letter codes are mislabelled** — `R` is bound to swap-hands (vanilla's F) and `G` to drop (vanilla's Q). Not broken, just named after the wrong keys, and the app's input grid shows the raw code as its button label. `F` was added correctly for the off-hand slot; see the input bugfix in the root `TODO.md` before renaming anything in that table, since both sides have to move together.
